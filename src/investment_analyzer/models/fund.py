...
class Fund:
    ...
    def name(self) -> str:
        """
        Return the actual fund name from local fund metadata.
        """

        from investment_analyzer.core.fund_metadata import (
            get_fund_name,
        )

        return get_fund_name(self.symbol)
    ...
    def load_data(self):
        ...
        self.data = (
            self.data.dropna(subset=["Date", "Adj Close"])
            .sort_values("Date")
